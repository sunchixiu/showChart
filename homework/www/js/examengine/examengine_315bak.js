var examengine = {
    ...
    bindevent: function () {
       ...
       // $("#answercarditem").html(examengine.htmlanswersheet);
        ...
        $(examengine.htmlcontainerid).find(".equestion").bind("click", function () {
            //移动端区别
            $(this).siblings().css("border", "1px solid #fff").find(".questionmenu").hide();
    ...
    handdlesubquestion: function (model) {
        ...
        var contentarr = examengine.replaceshowcontent( model.content).split('.</em>');
        var content = contentarr[0];
        if(contentarr.length > 1){
            content = contentarr[1];
        };
        html = html + "<div class='subtitle'><em class='subquestionnum'>" + model.index + ".</em>" + content + "</div>";
       ...
       // console.log(examengine.status)
        ...
        if (examengine.status == "view" || examengine.status == "edit"||examengine.status=="insert") {
          ...
          //  html = html + "<div class='answerline'></div>"
            ...
            if (examengine.status == "insert" || examengine.status == "edit") {
        ...
        else if (examengine.status == "piyue")
        {
            ...
            if (model.isobjective == false) {
                ...
                html = html + "         <div style='padding-bottom:8px; '>得分：<input type='text' data-fullscore='"+model.score+"' class='input pingscore' style='width:18px; padding: 4px 6px;' value='" + _piyuescore + "'> (总分：<label>"+model.score+"</label>)</div>";
                //移动端去掉textarea输入
            ...
            //html = html + "<input type='button' value='添加声音' onclick='addvoice(\"http://192.168.20.121/Public/developimages/ftb.mp3\", \"23\")'>" +
            //"<input type='button' value='添加文本' onclick='addtext(\"ddddddddddss\")'>" +
            //"<input type='button' value='点赞' onclick='adddelgood()'>";
        }
            ...
            if (!_isselectquestion) {
                ...
                {
                    //反显上传的图片
                    var _myanswercontent = _myanswer.result;
                    var _myanswerimage = '';
