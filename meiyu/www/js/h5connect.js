...
opt['refer']=document.domain;if(!targetId||!document.getElementById(targetId)){alert("没有指定有效播放器容器！");}else if(!opt['app_id']||!opt['file_id']){var video=opt['third_video'];if(video){if(false&&!video['duration']){}else if(!video['urls']){return alert("缺少视频地址信息，请补齐urls；");}else{return this.verifyDone(targetId,opt,listener);}}
