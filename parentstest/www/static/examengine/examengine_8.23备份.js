                ...
                if (sectionmodel.title != "") {
                    ...
                    if (examengine.status == "edit") {
                        ...
                        if(sectionmodel.title){
                            ...
                            questionlist = questionlist + "<div><span class='examsections_name'> </span></div>";
                        };
                    }
    ...
    bindevent: function () {
        ...
        if (examengine.issingledisplay) {
                    ...
                    function listentouch(prevnextstate){
                        ...
                        var tY = 0;
                        var istouch = false;
                        var testtop = $('#testpaper').offset().top;
                        function fnStart(ev){
                            if(!istouch){
                        ...
                        window.onresize = function(){
                            if($('#testpaper').offset().top != testtop){
                                //$('#testpaper').css('margin-top',parseInt($('#testpaper')[0].offsetTop)+Math.abs($('#testpaper').offset().top)+'px');
                                $('#testpaper').offset({'top': testtop});
                            };
                        };
                    };
                }
        ...
        if(examengine.issingledisplay){
            ...
            //设置答题卡题号按钮
            $("#answercardapp").find("a").on("click", function () {
                var index = parseInt($(this).attr('data-index'))-1;
                examengine.singleindex = index;
                if(index == 0 || index == $('.equestion').length - 1){
                    isprevnext = false;
                }else{
                    isprevnext = true;
                };
                listentouch(isprevnext);
                $('.equestion[data-index='+(index+1)+']').css('margin-left',0);
                $('.que-box').hide();
            });
        }else{
            ///重新生成答题卡
...
//            $("#answercarditem").find("[data-index='" + (parseInt( _subquestionindex,10)+1) + "']").click()
    ...
    handdlesubquestion: function (model) {
        ...
        html = html + "<div class='subtitle'><em class='subtitle_num'>" + model.index + ".</em><i class='subtitle_typename'>[" + model.typename + "]</i>" + examengine.replaceshowcontent(model.content) + "</div>";
