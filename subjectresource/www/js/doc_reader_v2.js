...
(function () {
    ...
    var pcXReaderUrl_v2 = 'http://static.bcedocument.com/reader/v2/pc/xreader/xreader.html';
    var pcReaderJsUrl_v2 = 'http://static.bcedocument.com/reader/v2/pc/api/api.min.js';
    var pcReaderCssUrl_v2 = 'http://static.bcedocument.com/reader/v2/pc/api/api.min.css';
    var wapXReaderUrl_v2 = 'http://static.bcedocument.com/reader/v2/wap/xreader/index.html';
    var wapReaderJsUrl_v2 = 'http://static.bcedocument.com/reader/v2/wap/api/api.min.js';
    var wapReaderCssUrl_v2 = 'css/api.min.css';
    ...
    function loadV2Document(container, option) {
        ...
        if (isMobile()) {
            ...
            option.JS_URL = wapReaderJsUrl_v2;
            option.CSS_URL = wapReaderCssUrl_v2;
            option.container = container;
            option.apiUrl = wapXReaderUrl_v2;
            option.tag = 'opencloud';
            console.log(option);
            loadCss(wapReaderCssUrl_v2);
            loadScript(wapReaderJsUrl_v2);
            window.Wenku=option;
        }
        else {
            if (!checkPcOption(option)) {
            ...
            option.JS_URL = pcReaderJsUrl_v2;
            option.CSS_URL = pcReaderCssUrl_v2;
            option.apiUrl = pcXReaderUrl_v2;
            option.doc_id = option.docId;
            option.container = container; // 解决pc上的拼写错误
            console.log(option);
            loadScript(pcReaderJsUrl_v2);
            window.Wenku = option;
        }
    }
