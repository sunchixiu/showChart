...
return!1;})();v.getFlashAble=function(){return v.ABLE_FLASH?(v.FLASH_VERSION<=10?'lowVersion':'able'):'';};var ableHlsJs=window.MediaSource&&window.MediaSource.isTypeSupported('video/mp4; codecs="avc1.42E01E,mp4a.40.2"')?true:false;var forceCheckHLS=function(){if(v.ANDROID&&!v.ABLE_H5_HLS){if(agent.substr(agent.indexOf('Android')+8,1)>=4){return true;}}
return false;};v.REQUIRE_HLS_JS=ableHlsJs&&!v.ABLE_H5_HLS&&!v.ABLE_H5_APPLE_HLS;v.getLivePriority=function(){if(v.IOS||v.ANDROID){if(forceCheckHLS()){v.ABLE_H5_HLS=true;}
return'h5';}
...
if(lPlayed>0){if(duration==0||(lPlayed+5<duration)){this.whenReadyToPlay=true;this.debug('���䲥��λ��:',lPlayed);this.system.setUrl(this.video_data['url']);this.system.play();}}},switchClarity:function(no){var me=this;var data=me.video_data;var system=me.system;if(data['resolutionNameNo']==no||!me.store.isInDefinition(no)){me.debug('��ǰ��֧�ָ�������л�',no);}else{me.debug('��ǰ�����',no);var video=me.video_data=me.store.getVideoInfo(constants.DEFINITION_NO_NAME[no|0]);var played=system.status.get_played();me.whenReadyToPlay=true;system.status.clear();system.status.setRememberKey(video.url);system.setUrl(video['url']);system.play(played);me.switchPlayTime=played;this.__filter_play();}},sdk_tecGet:function(attr,st){if(!this.store){return;}
...
this.debug('operate',method);switch(method){case("resize"):var $main=$('#'+this.store.getIDMain());$main.width(a).height(b);$main.parent().width(a).height(b);this.control.setResize();break;case("pause"):this.system.pause();break;case("resume"):this.toPlay();break;case("play"):this.toPlay(a);break;case("clarity"):this.switchClarity(b);break;case("capture"):break;case("destroy"):this.destroy();break;}},sdk_source:function(){if(this.store){var ret=$.extend({},this.store.getKeepArgs(),this.store.getMainOffset());delete ret.width;delete ret.height;this.destroy();return ret;}},sdk_barrage:function(barrageAryStr){},sdk_close_barrage:function(){}});});qcVideo('MediaPlayer_tpl',function(){return{'main':function(data){var __p=[],_p=function(s){__p.push(s)};_p('<style>\r\n\
    ...
    <div id="');_p(this.__escapeHtml(data.mainId));_p('" style="width:');_p(this.__escapeHtml(data.width));_p('px;height:');_p(this.__escapeHtml(data.height));_p('px"></div>');return __p.join("");},'patch':function(data){var __p=[],_p=function(s){__p.push(s)};_p('<div component="patch"></div>');return __p.join("");},'patch_image':function(data){var __p=[],_p=function(s){__p.push(s)};_p('<a style="');_p(this.__escapeHtml(data.astyle));_p('" href="');_p(this.__escapeHtml(data.link));_p('" ');_p(this.__escapeHtml((data.link.indexOf('javascript')>-1?'':' target="_blank"')));_p('><img style="');_p(this.__escapeHtml(data.istyle));_p('" src="');_p(this.__escapeHtml(data.url));_p('"/></a>');return __p.join("");},'logo':function(data){var __p=[],_p=function(s){__p.push(s)};_p('<div class="');_p(this.__escapeHtml(data.nick));_p('control-module" component="logo"></div>');return __p.join("");},'setting':function(data){var __p=[],_p=function(s){__p.push(s)};_p('<div class="');_p(this.__escapeHtml(data.nick));_p('setting" component="setting" style="display:none;">\r\n\
       ...
       </div>');return __p.join("");},'setting_definition':function(data){var __p=[],_p=function(s){__p.push(s)};_p('<!--�������-->\r\n\
   ...
   \r\n\
       ...
       </div>');return __p.join("");},'bottom':function(data){var __p=[],_p=function(s){__p.push(s)};var nick=data.nick;_p('    <!--�ײ�������-->\r\n\
       <div data-resize-module="bottom" class="');_p(this.__escapeHtml(nick));_p('control-bottom ');_p(this.__escapeHtml(nick));_p('control-bottom-flow" component="bottom_container" style="display:none;">\r\n\
           <div style="width:99%;height:36px;margin:0px auto;position:relative;  background: #000;">\r\n\
               ...
               <div class="');_p(this.__escapeHtml(nick));_p('progress-bar-container">\r\n\
                   <div class="');_p(this.__escapeHtml(nick));_p('progress-bar" aria-label="���Ż���" style="touch-action: none;');_p(this.__escapeHtml(data.version.IS_MOBILE?'display:none':''));_p('">\r\n\
...
_p('   /**ת�ջ�����**/');return __p.join("");},'style_bottom':function(data){var __p=[],_p=function(s){__p.push(s)};var i='#'+data.controller.id;var nick=data.nick;_p('        ');_p(this.__escapeHtml(i));_p(' button, ');_p(this.__escapeHtml(i));_p(' svg { margin: 0px; padding: 0px; }\r\n\
   \r\n\
...
if(rst){var map=me.getAllDefinition()[rst['definition']];me.debug(':getVideoInfo result:�����=='+map['resolution']+';���=='+map['resolutionName']+';sdk�����'+map['resolutionNameNo']);return util.merge({resolution:map['resolution'],resolutionName:map['resolutionName'],resolutionNameNo:map['resolutionNameNo']},rst);}}});});;qcVideo('PlayerSystem',function($,Base,interval,constants,util,FullScreenApi,version,JSON){var EVENT=constants.EVENT;var undefined=undefined;var getId=function(){return'video_id_'+(+new Date());};var EventAry='loadstart,suspend,abort,error,emptied,stalled,loadedmetadata,loadeddata,canplay,canplaythrough,playing,waiting,seeking,seeked,ended,durationchange,timeupdate,progress,play,pause,ratechange,volumechange'.split(',');var orientationMap={};var orientationChange=function(){var key;switch(window.orientation){case 180:for(key in orientationMap){if(orientationMap[key]&&orientationMap[key]['happenResize']){orientationMap[key]['happenResize'](true);}}
